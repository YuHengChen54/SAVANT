    ...
    with mlflow.start_run(
        ...
        for epoch in range(hyper_param["num_epochs"]):
            ...
            if train_loss.data < -1 and (epoch + 1) % 5 == 0:
                checkpoint_path = (
                    f"../model_with_a_physical_feature/model{hyper_param['model_index']}_checkpoints"
                )
            ...
            if current_loss > the_last_loss:  ### 測試比較少訓練的時候改這裡(註解掉這一行)
                ...
                if trigger_times >= patience:
                    # 往前縮排測試
                    path = "../model_with_a_physical_feature"
                    ...
                    for i in range(2): 
                        ...
                        optimizer = torch.optim.Adam(
                            [
                                ...
                                {"params": CNN_Physical_model.parameters()},
                        ...
                        training_loss, validation_loss = train_process(
                            ...
                            optimizer,
                            hyper_param,
                            experiment_name="SAVANT with nine physical features",
                            run_name=f"model {model_index} | physical feature: {physical_feature_list} | {loss_mode} | threshold: {chosen_intensity} | 20251202",
                            # run_name="test"
                        )
